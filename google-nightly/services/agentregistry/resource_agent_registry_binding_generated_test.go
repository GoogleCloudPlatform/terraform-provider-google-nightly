...
func testAccCheckAgentRegistryBindingDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, fmt.Sprintf("%s%s", transport_tpg.BaseUrl(agentregistry.Product, config), "projects/{{project}}/locations/{{location}}/bindings/{{binding_id}}"))
			if err != nil {
				return err
			}
